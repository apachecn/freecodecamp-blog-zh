# 面向非 Python 程序员的 Python 速成班——如何快速入门

> 原文：<https://www.freecodecamp.org/news/python-crash-course/>

本文面向已经有编程经验，想快速学习 Python 的人。

当我找不到一门在线课程或任何其他资源向已经了解其他编程语言的程序员教授 Python 时，我出于沮丧创建了这个资源。

在本课程中，我将涵盖 Python 的所有基础知识(深入的课程将在后面添加)，以便您可以快速入门这门语言。
...
要开始，请安装 Python 3。我推荐使用 VSCode 作为您的编辑器。它有许多要配置的扩展，因此您可以在几分钟内设置好您的环境。
...
如果你已经知道编程基础，你可能知道如何运行这个程序:p .用`.py`扩展名保存程序。然后运行`python hello.py`或`python3 hello.py`。

在这整个教程中，我们将跟随`python3`。`python2`将于 2020 年停产。所以我觉得还是用最新版本的好。
...
列表基本上类似于数组。在 Python 世界里，他们称之为`List`。它们是有序的。
...
元组就像列表一样，但是它们是不可变的。这意味着您不能添加或更新它们。你可以直接读取元素。记住，像列表一样，元组也是顺序的。
...
您可能已经知道了`if..else`语句是如何工作的。但是让我们来看一个例子:
...
原贴于本 Github 回购: [Python 速成班](https://github.com/srebalaji/python-crash-course)